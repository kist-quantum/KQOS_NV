...
class PODMR_LNV2(AveragerProgram):
    ...
    def initialize(self):
        ...
        # prepare lnv_ch
        freq_r = self.freq2reg(0, gen_ch = self.cfg["red_ch"], ro_ch = self.cfg["ro_memory"][0])
        phase_r = self.deg2reg(0, gen_ch = self.cfg["red_ch"])
        self.default_pulse_registers(ch = self.cfg["red_ch"], style = "const", mode = "oneshot", freq = freq_r, phase = phase_r)

        # prepare mw_ch
        freq_mw = self.freq2reg(self.cfg['x'], gen_ch = self.cfg["mw_ch"], ro_ch = self.cfg["ro_memory"][0])
    ...
    def body(self):
        ...
        self.set_pulse_registers(ch = self.cfg["red_ch"], gain = self.cfg["init_gain"], length = ns2cycle(self.cfg["init_width"]))
        self.pulse(ch = self.cfg["lnv_ch"])
        self.pulse(ch = self.cfg["red_ch"], t = ns2cycle(self.cfg['red_delay']))

        self.set_pulse_registers(ch = self.cfg["lnv_ch"], gain = self.cfg["free_gain"], length = ns2cycle(self.cfg["free_width"]))
        self.set_pulse_registers(ch = self.cfg["red_ch"], gain = self.cfg["free_gain"], length = ns2cycle(self.cfg["free_width"]))
        self.pulse(ch = self.cfg["lnv_ch"])
        self.pulse(ch = self.cfg["red_ch"], t = ns2cycle(self.cfg['red_delay']))

        self.set_pulse_registers(ch = self.cfg["lnv_ch"], gain = self.cfg["readout_gain"], length = ns2cycle(self.cfg["readout_width"]))
        self.set_pulse_registers(ch = self.cfg["red_ch"], gain = self.cfg["readout_gain"], length = ns2cycle(self.cfg["readout_width"]))
        self.pulse(ch = self.cfg["lnv_ch"])
        self.pulse(ch = self.cfg["red_ch"],t = ns2cycle(self.cfg['red_delay']))
        ...
        self.set_pulse_registers(ch = self.cfg["red_ch"], gain = self.cfg["after_gain"], length = ns2cycle(self.cfg["after_width"]))
        self.pulse(ch = self.cfg["lnv_ch"])
        self.pulse(ch = self.cfg["red_ch"],t = ns2cycle(self.cfg['red_delay']))
