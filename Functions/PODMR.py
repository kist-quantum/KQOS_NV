...
class PODMR(AveragerProgram):
    def initialize(self):
        self.declare_gen(ch=self.cfg["green_ch"])
        self.declare_gen(ch=self.cfg["mw_ch"])
        # self.declare_gen(ch=self.cfg["aux_green_ch"])   # what is this for?
        ...
        self.set_pulse_registers(ch=self.cfg["mw_ch"], style="const", mode="oneshot", length=ns2cycle(self.cfg['mw_len'])) #원본
    ...
    def body(self):
        ...
        self.synci(ns2cycle((self.cfg['spin_init_time'] + self.cfg['mw_offset'] + \
            self.cfg['dwell_left'] + self.cfg['mw_len'] + self.cfg['dwell_right'])))
        ...
        self.synci(ns2cycle((self.cfg['mw_offset'] + self.cfg['mw_len'] + self.cfg['dwell_right'])))
