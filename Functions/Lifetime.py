...
class Lifetime(AveragerProgram):
    ...
    def initialize(self):
        ...
        # envelope
        idata = np.array([self.cfg["idata_gain1"]]*self.cfg["idata_width1"] + [self.cfg["idata_gain2"]]*self.cfg["idata_width2"] + [self.cfg["idata_gain3"]]*self.cfg["idata_width3"] + [self.cfg["idata_gain4"]]*self.cfg["idata_width4"] + [self.cfg["idata_gain5"]]*self.cfg["idata_width5"])
        ...
        self.set_pulse_registers(ch = self.cfg["red_ch"], style = "const", mode = "oneshot", length = ns2cycle(self.cfg["red_len"]))
    ...
    def body(self):
        self.pulse(ch = self.cfg["green_ch"], 
                   t = ns2cycle(self.cfg["green_delay"]))
        self.pulse(ch = self.cfg["pulse_ch"],
                   t = ns2cycle(self.cfg["green_delay"] + self.cfg["init_len"] + self.cfg["mw_total_time"] + self.cfg["pulse_delay"]))
        self.pulse(ch = self.cfg["red_ch"],
                   t = ns2cycle(self.cfg["green_delay"] + self.cfg["init_len"] + self.cfg["mw_total_time"] + self.cfg["red_delay"]))
        self.trigger(pins = [1], 
                    t = ns2cycle(self.cfg["green_delay"] + self.cfg["init_len"] + self.cfg["mw_total_time"] + self.cfg["pulse_delay"] + self.cfg["apd_delay"]))
        self.trigger(pins = [0], 
                    t = ns2cycle(self.cfg["green_delay"] + self.cfg["init_len"] + self.cfg["mw_total_time"] + self.cfg["pulse_delay"] + self.cfg["apd_delay"]))
        

        for jj in range(int(self.cfg["mw_total_time"]/2/self.cfg["mw_len"])):

            freq1 = self.freq2reg(self.cfg["mw_freq1"], gen_ch = self.cfg["mw_ch"], ro_ch = 0)
            self.safe_regwi(rp = self.ch_page(self.cfg["mw_ch"]), reg = self.sreg(self.cfg["mw_ch"], "freq"), imm = freq1)
            self.pulse(ch = self.cfg["mw_ch"],
                       t = ns2cycle(self.cfg["green_delay"] + self.cfg["init_len"] + jj*2*self.cfg["mw_len"]))
            
            freq2 = self.freq2reg(self.cfg["mw_freq2"], gen_ch = self.cfg["mw_ch"], ro_ch = 0)
            self.safe_regwi(rp = self.ch_page(self.cfg["mw_ch"]), reg = self.sreg(self.cfg["mw_ch"], "freq"), imm = freq2)
            self.pulse(ch = self.cfg["mw_ch"],
                       t = ns2cycle(self.cfg["green_delay"] + self.cfg["init_len"] + (jj*2+1)*self.cfg["mw_len"]))
