...
class rabi_gr_ZPL(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.set_pulse_registers(ch=self.cfg["mw_ch"], style="const", mode="oneshot", length=ns2cycle(50)) #원본
        ...
        self.set_pulse_registers(ch=self.cfg["green_ch"], style="const", mode="oneshot", length=ns2cycle(50) ) # 원본
        # prepare Red Laser channel 
        ...
        self.set_pulse_registers(ch=self.cfg["red_ch"], style="const", mode="oneshot", length=ns2cycle(50) ) # 원본
        self.synci(200)
    ...
    def body(self):
        ...
        self.synci(ns2cycle((self.cfg['spin_init_time'] + self.cfg['dwell_left'] + self.cfg['x'] + self.cfg['dwell_right'])))
        ...
        self.pulse(ch=self.cfg["red_ch"], t=ns2cycle(self.cfg['laser_delay']))
        ...
        self.safe_regwi(rp=self.ch_page(self.cfg["green_ch"]), 
                        reg=self.sreg(self.cfg["green_ch"], "mode"),
                        mm=qasm.get_mode_code(self=self,
                                                length=ns2cycle(self.cfg['spin_init_time']),
                                                outsel='dds')
                        )
        ...
        self.synci(ns2cycle((self.cfg['x'] + self.cfg['dwell_right'])))
